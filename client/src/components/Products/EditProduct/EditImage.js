...
export default function EditImage() {
  ...
  const [image, setImage] = useState('')
    ...
    useEffect(() => {
        axios.get(`${baseUrl}/api/products/${productID}/images`)
        .then((response) => {
            console.log(response)
    ...
    return (
        <div>
  <Carousel activeIndex={index} onSelect={handleSelect} style={{ maxWidth: '500px', maxHeight: '200px', margin: '0 auto' }}>
    {images ? (
      images.map((image) => (
        <Carousel.Item>
  ...
  <div style={{ width: '200px', marginRight: 'auto', marginLeft: 'auto', marginTop: '100px' }}>
