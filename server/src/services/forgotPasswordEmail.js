...
module.exports.ForgotPasswordEmailSender = async (email) => {
  console.log(chalk.blue('Cron schedule Started'));

  const emailQuery = 'SELECT username, email FROM users WHERE email = ?';

  try {
    console.log('im inside try this is my email', email);
      ...
      const response = await Promise.all(
        rows.map(async (row) => {
          ...
          const data = await sendInBlue.sendTransacEmail({
            ...
            htmlContent: `<html><body><h1>Click this <a href="${baseUrl}/forgot">link</a> to change password.</h1></body></html>`,
