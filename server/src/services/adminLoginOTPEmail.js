...
module.exports.AdminOTPEmailSender = async (username) => {
  console.log(chalk.blue('Cron schedule Started'));

  const userQuery = 'SELECT username, email FROM admin WHERE username = ?';

  try {
    console.log('im inside try this is admin username', username);
    ...
    if (rows.length > 0) {
      ...
      const response = await Promise.all(
        rows.map(async (row) => {
          ...
          await pool.query('UPDATE admin SET otp = ? WHERE username = ?', [
            otp,
            username,
          ]);
          ...
          const data = await sendInBlue.sendTransacEmail({
            ...
            htmlContent: `<html><body><h1>This is your admin OTP ${otp}</h1></body></html>`,
