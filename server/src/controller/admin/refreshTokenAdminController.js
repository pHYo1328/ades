...
const handleRefreshToken = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const [updateResult] = await adminLoginServices.updateRefreshToken(
        newRefreshTokenArray,
        foundUser[0].userid
      );
      console.log(newRefreshTokenArray);
      console.log('Number of affected rows:', updateResult?.affectedRows);
      console.log('new refresh token of current admin', updateResult);
      ...
      const result = await adminLoginServices.updateRefreshToken(
        newRefreshTokenArray,
        foundUser.userid
      );
